...
class PCICMobileAppSupabaseUser extends BaseAuthUser {
  PCICMobileAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> pCICMobileAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PCICMobileAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
