...
class _CustomSignatureState extends State<CustomSignature>
        ...
        if (signatureBlob != null && signatureBlob.isNotEmpty) {
          try {
            ...
            if (decodedData.isNotEmpty) {
              ...
              _updateSignatureState(decodedData, 'Existing signature loaded');
            } else {
              throw Exception('Decoded signature data is empty');
            }
          } catch (e) {
            debugPrint('Error decoding or validating signature data: $e');
            _updateSignatureState(
                null, 'Failed to load existing signature: invalid data');
          }
        } else {
          _updateSignatureState(null, 'No existing signature found');
        }
      }
    } catch (e) {
      debugPrint('Error loading existing signature: $e');
      _updateSignatureState(
          null, 'Failed to load existing signature: ${e.toString()}');
    } finally {
      setStateIfMounted(() => _isLoading = false);
    }
  }

  void _updateSignatureState(Uint8List? data, String message) {
    if (_isDisposed) return;
    setStateIfMounted(() {
      ...
      if (data != null) {
        // Update the _signatureData and add points to the controller if data is available
        _signatureData = data;

        // Here, you can add your own logic to decode the signature data into points
        // and update the controller if necessary.
      } else {
        // If no data, ensure _signatureData is null
        _signatureData = null;
      }
    });
  }
  ...
  Future<void> _saveSignatureToDatabase(String base64Signature) async {
    ...
    try {
      ...
      // Mark the signature as saved in the app state
      FFAppState().signitureIsSaved = true;

      _updateSignatureState(
          base64.decode(base64Signature), 'Signature saved successfully');
    } catch (e) {
  ...
  Future<void> _clearSignature() async {
    ...
    // Update the UI
    _updateSignatureState(null, 'Signature cleared');

    try {
      // Clear the signature from SQLite
      if (widget.signatureFor == 'insured') {
        await SQLiteManager.instance.updatePPIRFormINSUREDSignatureBlob(
          taskId: widget.taskId,
          signatureBlob: null,
      ...
      // Optionally reset the saved state
      FFAppState().signitureIsSaved = false;
    } catch (e) {
  ...
  Future<void> _pickImageAndConvertToBase64() async {
    ...
    try {
      ...
      if (_isDisposed) return;
      _updateSignatureState(imageBytes, 'Signature image uploaded');
      await _saveSignatureToDatabase(base64String);
    } catch (e) {
      debugPrint('Error picking image: $e');
      if (!_isDisposed) {
        _updateSignatureState(null, 'Failed to pick image: ${e.toString()}');
      }
    } finally {
      if (!_isDisposed) {
        setStateIfMounted(() => _isLoading = false);
                  ...
                  Image.memory(
                    ...
                    errorBuilder: (context, error, stackTrace) {
                      ...
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        _updateSignatureState(
                            null, 'Failed to load signature: invalid data');
                      });
                      return const Center(
                      ...
                      if (_controller.isNotEmpty) {
                        ...
                        try {
                          ...
                          if (data != null && !_isDisposed) {
                            ...
                            await _saveSignatureToDatabase(base64String);
                            _updateSignatureState(
                                data, 'Drawn signature saved');
                          }
                        } catch (e) {
                          debugPrint('Error saving drawn signature: $e');
                          _updateSignatureState(null,
