...
class _MapBoxState extends State<MapBox> {
  ...
  final List<ll.LatLng> _routeCoordinates = [];
  ...
  double _latEstimate = 0.0;
  double _lonEstimate = 0.0;
  double _latErrorEstimate = 1.0;
  double _lonErrorEstimate = 1.0;
  static const double _q = 0.01; // Process noise
  static const double _r = 1.0; // Measurement noise
  ...
  Future<void> _initializeTileProvider() async {
    final stats = await FMTC.FMTCRoot.stats;
  ...
  Future<void> _initializeLocation() async {
    try {
      ...
      setState(() {
        ...
        _latEstimate = initialPosition.latitude;
        _lonEstimate = initialPosition.longitude;
      });
  ...
  void _startLocationStream() {
    ...
    _positionStreamSubscription =
      ...
      (Position position) {
        // Apply Kalman filter
        _updateKalmanFilter(position.latitude, position.longitude);

        setState(() {
          _currentLocation = ll.LatLng(_latEstimate, _lonEstimate);
          if (_isTracking) {
  ...
  void _updateKalmanFilter(double latMeasurement, double lonMeasurement) {
    // Update latitude estimate
    double latPredictedErrorEstimate = _latErrorEstimate + _q;
    double latKalmanGain =
        latPredictedErrorEstimate / (latPredictedErrorEstimate + _r);
    _latEstimate += latKalmanGain * (latMeasurement - _latEstimate);
    _latErrorEstimate = (1 - latKalmanGain) * latPredictedErrorEstimate;

    // Update longitude estimate
    double lonPredictedErrorEstimate = _lonErrorEstimate + _q;
    double lonKalmanGain =
        lonPredictedErrorEstimate / (lonPredictedErrorEstimate + _r);
    _lonEstimate += lonKalmanGain * (lonMeasurement - _lonEstimate);
    _lonErrorEstimate = (1 - lonKalmanGain) * lonPredictedErrorEstimate;
  }
            ...
            children: [
              ...
              CurrentLocationLayer(
                ...
                style: LocationMarkerStyle(
                  ...
                  accuracyCircleColor: Colors.green.withOpacity(0.3),
